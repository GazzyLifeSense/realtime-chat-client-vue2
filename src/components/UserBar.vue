<template>
    <div id="userbar">
        <div class="profile flex-center" @click="showDetail()">
            <a class="avatar flex-center">
                <img :src="getUserAvatar(user.avatar)" height=32 width=32 alt="头">
                <div class="online"></div>
            </a>
            <div class="username line">{{user.nickname}}</div>
            <UserDetail :config=userDetailConfig />
        </div>
        <div class="userbtn flex-center">
            <div class="notice flex-center" @click="getApplies">
...
export default {
    ...
    methods:{
        ...
        showDetail(){
            this.userDetailConfig = {display: 'flex'}
        },
    },
    mounted(){
...
#userbar{
    ...
    .profile:hover, .notice:hover, .setting:hover{
        background: #2E3237;
        border-radius: 6px;
        cursor: pointer;
        .online{
            border: solid 4px #2E3237;
        }
    }
}
